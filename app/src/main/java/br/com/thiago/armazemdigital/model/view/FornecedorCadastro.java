...
@Data
@NoArgsConstructor
@DatabaseView(value = "SELECT fornecedor.id as supplierId, fornecedor.name as supplierName, COUNT(DISTINCT produto.id) as countProduct FROM fornecedor LEFT JOIN fornecimento ON fornecedor.id = fornecimento.supplierId LEFT JOIN produto ON produto.id = fornecimento.productId GROUP BY fornecedor.id", viewName = "fornecedorCadastro")
